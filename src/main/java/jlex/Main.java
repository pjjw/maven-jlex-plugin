...
class CSpec {
	...
	Hashtable m_dfa_sets; /*
							 * Hashtable taking set of NFA states to
							 * corresponding DFA state, if the latter exists.
							 */
...
class CEmit {
	...
	private void print_details() {
		...
		for (i = 0; i < m_spec.m_row_map.length; ++i) {
			...
			if (null == accept) {
				out.println(" [nonaccepting]");
			} else {
				out.println(" [accepting, line "
			...
			if (CDTrans.F != state) {
				tr = true;
				out.print("\tgoto " + state + " on [" + ((char) 0));
			}
	...
	private void emit_helpers() throws java.io.IOException {
		...
		/* Function: yy_next */
		/*
		 * m_outstream.println("\tprivate int yy_next (int current,char
		 * lookahead) {"); m_outstream.println("\t\treturn
	...
	private void emit_driver() throws java.io.IOException {
		...
		if (NOT_EDBG) {
			m_outstream.println("\t\tjava.lang.out.println(\"Begin\");");
		}
		...
		if (NOT_EDBG) {
			m_outstream
					.println("java.lang.out.println(\"Current state: \""
							+ " + yy_state");
		...
		if (NOT_EDBG) {
			m_outstream
					.println("\t\t\tjava.lang.out.println(\"State = \""
							+ "+ yy_state);");
			m_outstream
					.println("\t\t\tjava.lang.out.println(\"Accepting status = \""
							+ "+ yy_this_accept);");
			m_outstream
					.println("\t\t\tjava.lang.out.println(\"Last accepting state = \""
							+ "+ yy_last_accept_state);");
			m_outstream
					.println("\t\t\tjava.lang.out.println(\"Next state = \""
							+ "+ yy_next_state);");
			m_outstream
					.println("\t\t\tjava.lang.out.println(\"Lookahead input = \""
							+ "+ ((char) yy_lookahead));");
		}
		 ...
		 * m_outstream.println("\t\t\t\t\tyy_this_accept = yy_acpt[yy_state];");
		 * m_outstream.println("\t\t\t\t\tif (YY_NOT_ACCEPT != yy_this_accept)
		 * {"); m_outstream.println("\t\t\t\t\t\tyy_last_accept_state =
		 * yy_state;"); m_outstream.println("\t\t\t\t\t}");
		 */
...
class CMakeNfa {
	...
	void thompson(CLexGen lexGen, CSpec spec, CInput input)
		 ...
		 */
		/*
		 * m_spec.m_current_token = CLexGen.EOS; m_lexGen.advance();
		 */
	...
	private CNfa machine() throws java.io.IOException {
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.enter("machine", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.leave("machine", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
	...
	private CNfa rule() throws java.io.IOException {
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.enter("rule", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
		...
		if (end == null)
			CError.parse_error(CError.E_ZERO, m_input.m_line_number, out);
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.leave("rule", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
	...
	private void expr(CNfaPair pair) throws java.io.IOException {
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.enter("expr", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.leave("expr", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
	}
	...
	private void cat_expr(CNfaPair pair) throws java.io.IOException {
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.enter("cat_expr", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.leave("cat_expr", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
	}
	...
	private boolean first_in_cat(int token) {
		...
		case CLexGen.OPTIONAL:
			CError.parse_error(CError.E_CLOSE, m_input.m_line_number, out);
			return false;

		case CLexGen.CCL_END:
			CError.parse_error(CError.E_BRACKET, m_input.m_line_number, out);
			return false;

		case CLexGen.AT_BOL:
			CError.parse_error(CError.E_BOL, m_input.m_line_number, out);
			return false;
	...
	private void factor(CNfaPair pair) throws java.io.IOException {
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.enter("factor", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.leave("factor", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
	}
	...
	private void term(CNfaPair pair) throws java.io.IOException {
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.enter("term", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
		...
		if (CLexGen.OPEN_PAREN == m_spec.m_current_token) {
			...
			if (CLexGen.CLOSE_PAREN == m_spec.m_current_token) {
				...
				CError.parse_error(CError.E_SYNTAX, m_input.m_line_number, out);
			}
		} else {
			start = CAlloc.newCNfa(m_spec);
			pair.m_start = start;
			...
			if (false == (CLexGen.ANY == m_spec.m_current_token
					|| CLexGen.CCL_START == m_spec.m_current_token || (m_spec.m_ignorecase && isAlphaL))) {
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.leave("term", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
	}
	...
	private void dodash(CSet set) throws java.io.IOException {
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.enter("dodash", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
		...
		if (CUtility.DESCENT_DEBUG) {
			CUtility.leave("dodash", m_spec.m_lexeme, m_spec.m_current_token, out);
		}
	}
}
...
class CSimplifyNfa {
	...
	private void computeClasses(CSpec m_spec) {
		...
		out.print("Working on character classes.");
		for (Enumeration e = m_spec.m_nfa_states.elements(); e
				.hasMoreElements();) {
			CNfa nfa = (CNfa) e.nextElement();
			if (nfa.m_edge == CNfa.EMPTY || nfa.m_edge == CNfa.EPSILON)
...
class CMinimize {
	...
	private void pgroups() {
		...
		for (i = 0; i < dtrans_size; ++i) {
			out.println("\tstate " + i + " is in group " + m_ingroup[i]);
		}
	}
}
...
class CNfa2Dfa {
	...
	void make_dfa(CLexGen lexGen, CSpec spec) {
		...
		if (m_spec.m_verbose && true == CUtility.OLD_DUMP_DEBUG) {
			out.println(m_spec.m_dfa_states.size()
	...
	/* throws java.lang.CloneNotSupportedException */
	{
		...
		out.print("Working on DFA states.");
		...
		while (false == nfa_stack.empty()) {
			...
			if (CUtility.OLD_DUMP_DEBUG) {
				if (null != state.m_accept) {
					out.println("Looking at accepting state "
							+ state.m_label
							+ " with <"
			...
			if (null != state.m_accept && state.m_label < bunch.m_accept_index) {
				...
				if (CUtility.OLD_DUMP_DEBUG) {
					out.println("Found accepting state "
							+ state.m_label
							+ " with <"
							+ (new String(state.m_accept.m_action, 0,
									state.m_accept.m_action_read)) + ">");
	...
	void move(Vector nfa_set, SparseBitSet nfa_bit, int b, CBunch bunch) {
		...
		for (index = 0; index < size; ++index) {
			...
			if (b == state.m_edge
				...
				bunch.m_nfa_set.addElement(state.m_next);
				/*
				 * out.println("Size of bitset: " +
				 * bunch.m_nfa_bit.size()); out.println("Reference index: " +
	...
	private void sortStates(Vector nfa_set) {
		...
		if (CUtility.OLD_DEBUG) {
			out.print("NFA vector indices: ");

			for (index = 0; index < size; ++index) {
				elem = (CNfa) nfa_set.elementAt(index);
				out.print(elem.m_label + " ");
			}
			out.println("");;
		}
	...
	private CDfa get_unmarked() {
		...
		while (m_unmarked_dfa < size) {
			...
			if (false == dfa.m_mark) {
				...
				if (m_spec.m_verbose && true == CUtility.OLD_DUMP_DEBUG) {
					out.println("---------------");
					out.print("working on DFA state " + m_unmarked_dfa
							+ " = NFA states: ");
...
public class Main {
	...
	public static void main(String arg[]) throws java.io.IOException {
		...
		if (arg.length < 2) {
			System.out
					.println("Usage: jlex.Main <input filename> <output filename>");
			return;
		}
...
class CUtility {
	...
	static void enter(String descent, char lexeme, int token, LexOutput out) {
		out.println("Entering " + descent + " [lexeme: " + lexeme
				+ "] [token: " + token + "]");
	}
	...
	static void leave(String descent, char lexeme, int token, LexOutput out) {
		out.println("Leaving " + descent + " [lexeme:" + lexeme
				+ "] [token:" + token + "]");
	}
	...
	static int bytencmp(byte a[], int a_first, byte b[], int b_first, int n) {
		...
		for (elem = 0; elem < n; ++elem) {
			/*
			 * out.print((char) a[a_first + elem]);
			 * out.print((char) b[b_first + elem]);
			 */
...
class CError {
	 ...
	 **************************************************************************/
	static void impos(String message, LexOutput out) {
		out.println("JLex Error: " + message);
	}
	 ...
	 **************************************************************************/
	static void parse_error(int error_code, int line_number, LexOutput out) {
		out.println("Error: Parse error at line " + line_number + ".");
		out.println("Description: " + errmsg[error_code]);
...
class CNfa {
	...
	CNfa m_next2; /*
					 * Another state with type == EPSILON and null if not used.
					 * The NFA construction should result in two outgoing edges
					 * only if both are EPSILON edges.
					 */
...
class CLexGen {
	 ...
	 * Function: CLexGen
	 **************************************************************************/
	CLexGen(String filename, String outputFilename, LexOutput out)
		...
		if (null == m_outstream) {
			out.println("Error: Unable to open output file " + filename
					+ ".java.");
			return;
		}
		...
		m_nfa2dfa = new CNfa2Dfa(out);
		m_minimize = new CMinimize(out);
		m_makeNfa = new CMakeNfa(out);
		m_simplifyNfa = new CSimplifyNfa(out);

		m_emit = new CEmit(out);
	...
	void generate() throws java.io.IOException, java.io.FileNotFoundException {
		if (false == m_init_flag) {
			CError.parse_error(CError.E_INIT, 0, out);
		}
		...
		if (m_spec.m_verbose) {
			out.println("Processing first section -- user code.");
		}
		userCode();
		if (m_input.m_eof_reached) {
			CError.parse_error(CError.E_EOF, m_input.m_line_number, out);
		}

		if (m_spec.m_verbose) {
			out.println("Processing second section -- "
					+ "JLex declarations.");
		}
		userDeclare();
		if (m_input.m_eof_reached) {
		...
		if (m_spec.m_verbose) {
			out.println("Processing third section -- lexical rules.");
		}
		userRules();
		if (CUtility.DO_DEBUG) {
			print_header();
		...
		if (m_spec.m_verbose) {
			out.println("Outputting lexical analyzer code.");
		}
	...
	private void userCode() throws java.io.IOException {
		...
		if (m_input.m_eof_reached) {
			CError.parse_error(CError.E_EOF, 0, out);
		}
	...
	private char[] getName() {
		...
		if (m_input.m_line_index >= m_input.m_line_read) {
			CError.parse_error(CError.E_DIRECT, 0, out);
		}
	...
	private char[] packCode(char start_dir[], char end_dir[], char prev_code[],
		...
		if (0 != CUtility.charncmp(m_input.m_line, 0, start_dir, 0,
				start_dir.length - 1)) {
			CError.parse_error(CError.E_INTERNAL, 0, out);
		}
		...
		while (false == m_input.getLine()) {
			...
			if ('%' == m_input.m_line[0]) {
				...
				if (1 >= m_input.m_line_read) {
					CError.parse_error(CError.E_DIRECT, m_input.m_line_number, out);
					continue;
				}
			...
			if (CUtility.OLD_DEBUG) {
				...
				out.println(new String(m_input.m_line, 0,
						m_input.m_line_read));
			}
		}
	}
	...
	private void userRules() throws java.io.IOException {
		...
		if (m_spec.m_verbose) {
			out.println("Creating NFA machine representation.");
		}
		...
		if (m_spec.m_verbose) {
			out.println("Creating DFA transition table.");
		}
		...
		if (m_spec.m_verbose) {
			out.println("Minimizing DFA transition table.");
		}
	...
	void print_nfa() {
		...
		for (elem = 0; elem < size; ++elem) {
			...
			out.print("Nfa state " + plab(nfa) + ": ");

			if (null == nfa.m_next) {
				out.print("(TERMINAL)");
			} else {
				out.print("--> " + plab(nfa.m_next));
				out.print("--> " + plab(nfa.m_next2));
				...
				default:
					out.print(" " + interp_int(nfa.m_edge));
					break;
				}
			}

			if (0 == elem) {
				out.print(" (START STATE)");
			}
		...
		while (states.hasMoreElements()) {
			...
			out.println("State \"" + state + "\" has identifying index "
					+ index.toString() + ".");
			out.print("\tStart states of matching rules: ");
			...
			for (j = 0; j < vsize; ++j) {
				...
				out.print(m_spec.m_nfa_states.indexOf(nfa) + " ");
			}
	...
	SparseBitSet getStates() throws java.io.IOException {
		...
		if ('<' == m_input.m_line[m_input.m_line_index]) {
			...
			while (true) {
				...
				if (null == index) {
					/* Uninitialized state. */
					out.println("Uninitialized State Name: " + name);
					CError.parse_error(CError.E_STATE, m_input.m_line_number, out);
				}
	...
	private boolean expandMacro() {
		...
		/* Check for macro. */
		if ('{' != m_input.m_line[m_input.m_line_index]) {
			CError.parse_error(CError.E_INTERNAL, m_input.m_line_number, out);
			return ERROR;
		}
		...
		if (elem >= m_input.m_line_read) {
			CError.impos("Unfinished macro name", out);
			return ERROR;
		}
		...
		/* Check macro name. */
		if (0 == count_name) {
			CError.impos("Nonexistent macro name", out);
			return ERROR;
		}
		...
		if (null == def) {
			...
			out.println("Error: Undefined macro \"" + name + "\".");
			CError.parse_error(CError.E_NOMAC, m_input.m_line_number, out);
			return ERROR;
		}
		if (CUtility.OLD_DUMP_DEBUG) {
			out.println("expanded escape: " + def);
		}
	...
	private void saveMacro() {
		...
		while (false == CUtility.isspace(m_input.m_line[elem])
			...
			if (elem >= m_input.m_line_read) {
				...
				CError.parse_error(CError.E_MACDEF, m_input.m_line_number, out);
			}
		}
		count_name = elem - start_name;
		...
		if (CUtility.OLD_DEBUG) {
			out.println("macro name \""
					+ new String(m_input.m_line, start_name, count_name)
					+ "\".");
			out.println("macro definition \""
	...
	private void saveStates() {
		...
		while (m_input.m_line_index < m_input.m_line_read) {
			if (CUtility.OLD_DEBUG) {
				out.println("line read " + m_input.m_line_read
			...
			if (CUtility.OLD_DEBUG) {
				out.println("State name \""
						...
						+ "\".");
				out.println("Integer index \"" + m_spec.m_states.size()
						+ "\".");
			}
	...
	private char expandEscape() {
		...
		if ('\\' != m_input.m_line[m_input.m_line_index]) {
			...
			case '^':
				...
				if (r < '@' || r > 'Z') // non-fatal
					CError.parse_error(CError.E_BADCTRL, m_input.m_line_number, out);
				r = (char) (r - '@');
				++m_input.m_line_index;
				return r;
			...
			case 'x':
				...
				for (int i = 0; i < (unicode_escape ? 4 : 2); i++)
					if (CUtility
						...
						r = (char) (r << 4);
						r = (char) (r | CUtility
								.hex2bin(m_input.m_line[m_input.m_line_index], out));
						++m_input.m_line_index;
					} else
						break;
				...
				if (false == CUtility
					...
					for (int i = 0; i < 3; i++)
						if (CUtility
							...
							r = (char) (r << 3);
							r = (char) (r | CUtility
									.oct2bin(m_input.m_line[m_input.m_line_index], out));
							++m_input.m_line_index;
						} else
							break;
				}
	...
	CAccept packAccept() throws java.io.IOException {
		...
		/* Look for brackets. */
		if ('{' != m_input.m_line[m_input.m_line_index]) {
			CError.parse_error(CError.E_BRACE, m_input.m_line_number, out);
		}
		...
		while (true) {
			...
			while (m_input.m_line_index >= m_input.m_line_read) {
				...
				if (insinglequotes || indoublequotes) { // non-fatal
					CError.parse_error(CError.E_NEWLINE, m_input.m_line_number, out);
		...
		if (CUtility.DESCENT_DEBUG) {
			out.print("Accepting action:");
			out.println(new String(accept.m_action, 0,
					accept.m_action_read));
		}
	...
	int advance() throws java.io.IOException {
		...
		/*
		 * if (m_input.m_line_index > m_input.m_line_read) {
		 * out.println("m_input.m_line_index = " + m_input.m_line_index);
		 * out.println("m_input.m_line_read = " + m_input.m_line_read);
		 * CUtility.ASSERT(m_input.m_line_index <= m_input.m_line_read); }
		 */
		...
		if (CUtility.FOODEBUG) {
			out.println("Lexeme: " + m_spec.m_lexeme + "\tToken: "
	...
	private void details() {
		...
		while (names.hasMoreElements()) {
			...
			out.println("Macro name \"" + name + "\" has definition \""
					+ def + "\".");
		}
		...
		out.println("");;
		out.println("\t** Character Counting **");
		if (false == m_spec.m_count_chars) {
			out.println("Character counting is off.");
		} else {
			if (CUtility.DEBUG) {
				CUtility.ASSERT(m_spec.m_count_lines);
			}

			out.println("Character counting is on.");
		}

		out.println("");;
		out.println("\t** Line Counting **");
		if (false == m_spec.m_count_lines) {
			out.println("Line counting is off.");
		} else {
			if (CUtility.DEBUG) {
				CUtility.ASSERT(m_spec.m_count_lines);
			}

			out.println("Line counting is on.");
		}

		out.println("");;
		out.println("\t** Operating System Specificity **");
		if (false == m_spec.m_unix) {
			out.println("Not generating UNIX-specific code.");
			out.println("(This means that \"\\r\\n\" is a "
					+ "newline, rather than \"\\n\".)");
		} else {
			out.println("Generating UNIX-specific code.");
			out.println("(This means that \"\\n\" is a "
					+ "newline, rather than \"\\r\\n\".)");
		}

		out.println("");;
		out.println("\t** Java CUP Compatibility **");
		if (false == m_spec.m_cup_compatible) {
			out.println("Generating CUP compatible code.");
			out.println("(Scanner implements "
					+ "java_cup.runtime.Scanner.)");
		} else {
			out.println("Not generating CUP compatible code.");
		}

		if (CUtility.FOODEBUG) {
			if (null != m_spec.m_nfa_states && null != m_spec.m_nfa_start) {
				out.println("");;
				out.println("\t** NFA machine **");
				print_nfa();
			}
		}
		...
		if (null != m_spec.m_dtrans_vector) {
			out.println("");;
			out.println("\t** DFA transition table **");
			/* print_header(); */
		}

		/*
		 * if (null != m_spec.m_accept_vector && null != m_spec.m_anchor_array) {
		 * out.println("");; out.println("\t** Accept States and
	...
	private void print_header() {
		...
		while (states.hasMoreElements()) {
			...
			i = index.intValue();
			if (CDTrans.F != m_spec.m_state_dtrans[i]) {
				out.println("\tStart index in transition table: "
						+ m_spec.m_state_dtrans[i]);
			} else {
				out.println("\tNo associated transition states.");
			}
		}
		...
		for (i = 0; i < m_spec.m_dtrans_vector.size(); ++i) {
			...
			if (null == m_spec.m_accept_vector && null == m_spec.m_anchor_array) {
				if (null == dtrans.m_accept) {
					out.print(" * State " + i + " [nonaccepting]");
				} else {
					out.print(" * State "
							+ i
							+ " [accepting, line "
			...
			for (j = 0; j < m_spec.m_dtrans_ncols; ++j) {
				if (CDTrans.F != dtrans.m_dtrans[j]) {
					if (last_transition != dtrans.m_dtrans[j]) {
						out.println("");;
						out.print(" *    goto " + dtrans.m_dtrans[j]
								+ " on ");
						chars_printed = 0;
					}
